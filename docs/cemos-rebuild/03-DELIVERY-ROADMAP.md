...
- **Faz 4C/D — Learn + Obsidian:** (4C) Learn UI + NotebookLM/manuel transcript → Pack → atomik not/zihin haritası/task/review (`learning/pipeline` + `components/learn/*` + `/api/learn/jobs/[id]/advance` durum makinesi); (4D) Obsidian GitHub/local export uçtan uca canlı doğrulama + ABSORBED ekran emekliliği (compatibility kanıtı). Mevcut Learn/Obsidian/Boards altyapısı yeniden İCAT EDİLMEZ.
