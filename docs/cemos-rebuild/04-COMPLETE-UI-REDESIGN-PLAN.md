...
> **Phase 4D (ADR-043) — fiziksel emeklilik UYGULANDI (2026-07-19):** parity-audit sonrası 10 runtime-DARK component fiziksel SİLİNDİ (grep ile 0 static/dynamic/string-id importer): `DailyQueueTab`/`KeywordLibraryTab`/`PromptKutuphanesiTab`/`PatternLibraryTab`/`LearnDashboardTab`/`FlowRadarTab`/`SourceIntelligenceTab`/`ViralLibraryTab` + orphan `agent/FlowList`+`agent/TweetCard`. Yukarıdaki TAB_ALIASES + store v9 migration + `normalizeTabId` + AppShell unknown-guard DEĞİŞMEDEN korundu → tüm eski id'ler canonical ekrana çözülmeye devam eder (e2e ile kanıtlı). `ViralLibraryTab`'ın localStorage→DB `savedTweets` drenajı `savedTweetsMigration.ts` (saf/testli) + AppShell hydration-farkında mount bootstrap'ına taşındı (idempotent; veri kaybı yok). **KORUNDU (silinmedi):** `InstagramTab` + `CompetitorRadarSection` + `ReelsDossierSection` — IG rakip-radar/reels UI'sinin canonical evi yok, silmek 2 ek feature dosyasını cascade-orphan ederdi; re-home bekliyor. UI silmek veri modelini silmez → tüm `/api/*` + Prisma modelleri korundu.
