...
> **Katalog doğrulama:** `https://openrouter.ai/api/v1/models` (JSON API), **2026-07-08**. Slug'lar ve fiyatlar bu tarihe pinli; her build başında yeniden doğrula (`AiModelSnapshot` haftalık drift kontrolü V1'de otomatikleşir). Brief'teki hipotez adları (`gpt-5.4-mini/nano`, `gemini-3-flash-preview`, `deepseek-v3.2`) **gerçek slug değil** — aşağıdaki doğrulanmış karşılıklara map'lendi.
> **Kurallar:** primary = pinned dated slug; floating (`-latest`/`-fast`/`fable`) yalnız fallback; her primary'nin farklı-sağlayıcı fallback zinciri var; preset katmanı mevcut rol registry'sinin (`model-config.ts`) ÜZERİNE oturur, yeniden yazmaz; tüm çağrılar `generateJsonGated` üzerinden.

---

## 1. Doğrulanmış katalog (2026-07-08, $/M input/output)

| Pinned slug (primary kullanım) | Ctx | In/Out | Not |
|---|---|---|---|
| `anthropic/claude-sonnet-5-20260630` | 1M | $2/$10 | Writer — sonnet-4-5'ten hem ucuz hem iyi |
| `anthropic/claude-4.8-opus-20260528` | 1M | $5/$25 | Yalnız V2 escalation |
| `openai/gpt-5.5-20260423` | 1.05M | $5/$30 | **Final judge primary (C3)** + writer fallback |
| `openai/gpt-5.5-pro-20260423` | 1.05M | $30/$180 | Kaçın; son çare |
| `google/gemini-3.5-flash-20260519` | 1M | $1.5/$9 | Research/multimodal iş atı |
| `google/gemini-3.1-flash-lite-20260507` | 1M | $0.25/$1.5 | Extract/classify iş atı |
| `google/gemini-3-pro-image-20260528` | 65k | $2/$12 | Görsel (fal birincil kalır) |
| `deepseek/deepseek-v4-flash-20260423` | 1M | $0.09/$0.18 | Mutlak taban prefilter |
| `deepseek/deepseek-v4-pro-20260423` | 1M | $0.44/$0.87 | Ucuz büyük-ctx consolidation |
| `openai/text-embedding-3-small` | — | $0.02/M | Embedding default (MVP) |
| Floating (yalnız fallback): `anthropic/claude-sonnet-latest`, `openai/gpt-mini-latest` ($0.75/$4.50), `google/gemini-pro-latest` ($2/$12), `google/gemini-flash-latest` | | | `claude-haiku-latest` pinned'i yok → **kullanma** (`unverified`) |

**Eski→yeni map (`model-config.ts` default'ları):** `gemini-2.5-pro`→`gemini-3.5-flash-20260519` · `gemini-2.5-flash`→`gemini-3.1-flash-lite-20260507` · `claude-sonnet-4-5`→`claude-sonnet-5-20260630` · `deepseek-chat(:free)`→`deepseek-v4-flash-20260423` · `gpt-4o(-mini)`→`gpt-5.5-20260423`/`gpt-mini-latest`.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `cemos-fast-extract` | cheapWriter | gemini-3.1-flash-lite-20260507 | deepseek-v4-flash → gemini-3.5-flash | price | none | 0 | json_schema | auto | allow | `extract_` | 15s/1 |
| `cemos-budget-batch` | cheapWriter | deepseek-v4-flash-20260423 | gemini-3.1-flash-lite | price | none | 0 | json_schema | auto | allow | `prefilter_` | 12s/1 |
| `cemos-research` | qualityJudge | gemini-3.5-flash-20260519 | deepseek-v4-pro → gpt-5.5 | price | low | 0.3 | json_schema | auto | allow | `research_` | 40s/1 |
| `cemos-multimodal-audit` | qualityJudge | gemini-3.5-flash-20260519 | gemini-pro-latest → gpt-5.5 | price | low | 0.2 | json_schema | auto | allow | `audit_` | 45s/1 |
| `cemos-memory` | cheapWriter | deepseek-v4-pro-20260423 (+ text-embedding-3-small) | gemini-3.5-flash; local-hash embed | price | none | 0.1 | json_schema | auto | **deny** | `memory_` | 30s/1 |
| `cemos-writer` | creativeWriter | **claude-sonnet-5-20260630** | gpt-5.5 → gemini-pro-latest | order:[anthropic] | medium | 0.9 | none | **anthropic-breakpoint** | **deny** | `writer_` | 45s/1 |
| `cemos-strategist` | qualityJudge | claude-sonnet-5-20260630 | gpt-5.5 → gemini-3.5-flash | order:[anthropic] | high | 0.4 | json_schema | anthropic-breakpoint | **deny** | `strategy_` | 60s/1 |
| `cemos-final-judge` | viralJudge/finalEditor | **openai/gpt-5.5-20260423** ← C3: writer'la KARŞI aile | gemini-3.5-flash → claude-sonnet-5 (son çare) | order:[openai] | low | 0.2 | json_schema | auto (OpenAI) | **deny** | `judge_` | 30s/1 |
| `cemos-image-concept` | creativeWriter | gemini-3.5-flash-20260519 (konsept metni) | claude-sonnet-5 | price | low | 0.7 | json_schema | auto | **deny** | `image_` | 30s/1 |

**Ortak:** tools = none/read-only (yayın aracı ASLA); escalation V2 flag'li (`writer→opus-4.8`, `judge→opus-4.8`, günlük cap + per-call gate); startup lint primary'nin pinned olduğunu doğrular (regex `-\d{8}$`), floating primary CI'ı kırar. **Writer-ailesi ≠ judge-ailesi registry testi zorunlu** (Anthropic writer / OpenAI judge — Panickssery NeurIPS 2024 self-preference savunması).
...
| 11 | Competitor multimodal | O | multimodal-audit | gemini-3.5-flash | gemini-pro-latest | low | yok | json_schema | auto | **evet** (operatör capture başlatır) | ~$0.02/görsel |
...
| 19 | Caption | Y | writer | claude-sonnet-5 | gemini-pro-latest | medium | yok | none | breakpoint | **evet** (edit-gate) | ~$0.01 |
...
| 25 | Visual audit | O | multimodal-audit | gemini-3.5-flash | gemini-pro-latest | low | yok | json_schema | auto | hayır | ~$0.02 |
...
- [ ] Her preset primary'si pinned + katalog snapshot'ında mevcut (test).
